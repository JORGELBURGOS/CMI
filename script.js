...
const API_KEY = 'AIzaSyCSKa4k3fyULiZmH7ZZKkEYz2NWZ5EI2qI'; // API key pública (puedes crear una propia)
...
function obtenerUnidad(indicador) {
  // Financieros
...
function obtenerResponsable(indicador) {
...
async function obtenerDatosDashboard(filtros) {
  try {
    ...
    if (actualCol === -1) {
      return { error: `Período actual '${periodoActual}' no encontrado.` };
    }
    
    if (compareCol === -1) {
      return { error: `Período de comparación '${periodoComparacion}' no encontrado.` };
    }
    ...
    for (let i = 1; i < sheetData.length; i++) {
      ...
      results.push({
        ...
        formula: row[3],
        unidad: obtenerUnidad(row[1]),
        responsable: obtenerResponsable(row[1]),
        meta: metaValue,
        actual: actualValue,
...
// Función para mostrar el modal (simplificada)
